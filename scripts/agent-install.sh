...
curl -H "Agent-Version-Control: on" -L $MANAGERURL/software/agent/${runningPlatform}${majorVersion}/${archType}/$package?tenantID=15532 -o /tmp/$package $CURLOPTIONS
echo "$MANAGERURL/software/agent/${runningPlatform}${majorVersion}/${archType}/$package?tenantID=15532"
echo "/tmp/$package $CURLOPTIONS"
...
echo clean up install package file
rm /tmp/agent.*